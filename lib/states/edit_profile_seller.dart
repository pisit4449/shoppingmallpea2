...
import 'package:shoppingmallpea2/main.dart';
...
class _EditProfileSellerState extends State<EditProfileSeller> {
  ...
  Future<Null> processEditProfileSeller() async {
    print('###processEdtiProfileSeller Work');
    ...
    if (formKey.currentState!.validate()) {
      if (file == null) {
        ...
        await Dio().post(apiSaveAvatar, data: formData).then((value) {
          print('## Upload Succes');
          String pathAvatar = '/shoppingmallnew/avatar/$nameFile';
  ...
  Future<Null> editValueToMySQL(String pathAvatar) async {
    ...
    String apiEditProfile = '${MyConstant.domain}/shoppingmallnew/editProfileSellerWhereId.php?isAdd=true&id=${userModel!.id}&name=${nameController.text}&address=${addressController.text}&phone=${phoneController.text}&avatar=$pathAvatar&lat=${latLng!.latitude}&lng=${latLng!.longitude}';
